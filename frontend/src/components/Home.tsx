...
const Home: React.FC<HomeProps> = ({ onSectionChange }) => {
  ...
  const statsData = [
    {
      ...
      value: stats.totalFacturas.toLocaleString(),
      change: '+0%',
    ...
    },
    {
      ...
      change: `${stats.cambioPorcentual >= 0 ? '+' : ''}${stats.cambioPorcentual}%`,
    ...
    },
    {
      ...
      value: stats.usuariosPendientes.toString(),
      change: '-0%',
  ...
  const recentActivities = stats.facturasRecientes.map((factura, index) => ({
    ...
    description: `Factura #${factura.numero} por ${formatCurrency(factura.monto)}`,
    time: new Date(factura.fecha).toLocaleDateString('es-ES'),
